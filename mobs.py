...
class Mob:
    ...
    def check_collision(self, get_block_func):
        # Без try/except – ошибка всплывёт в move_x/move_y, а оттуда в update
...
class Zombie(Mob):
    ...
    def move_x(self, dt, get_block_func):
        # Без try/except – ошибка всплывёт в update
        self.x += self.vx * dt
        if self.check_collision(get_block_func):
            self.x -= self.vx * dt
            if self.is_grounded:
    ...
    def update(self, player_x, player_y, is_night, player, dt):
        try:
            ...
            if self.spawn_timer >= 2.0:
                ...
                # ----- Определяем общий лимит мобов на основе MOB_SPAWN_LIMITS -----
                total_limit = MAX_MOBS  # значение по умолчанию
                if MOB_SPAWN_LIMITS:
                    total_limit = 0
                    ...
                    # Если лимит оказался равен нулю, используем MAX_MOBS
                    if total_limit == 0:
                        total_limit = MAX_MOBS

                if len(self.mobs) < total_limit:
                    # ----- Используем новые радиусы спавна -----
                    radius_min = MOB_SPAWN_RADIUS_MIN
                    radius_max = MOB_SPAWN_RADIUS_MAX
                    # Fallback на случай, если константы не заданы
                    ...
                    # ----- Выбор типа моба (оставляем как было) -----
                    if is_night:
                        r = random.random()
                        if r < 0.4:
                            mob = Zombie(sx, sy)
