...
class GameRenderer:
    def __init__(self, game):
        ...
        # ---------- Локальные копии данных для отрисовки ----------
        self.player_hp = game.player.hp
        self.player_max_hp = game.player.max_hp
        self.inventory_slots = game.inventory.slots[:]
        self.selected_slot = game.inventory.selected_slot
        self.dragged_slot = game.inventory.dragged_slot
        ...
        # ---------- СПИСОК ПОДПИСОК ДЛЯ ОТПИСКИ ----------
        self._subscriptions = []

        # Подписка на события через обёртку
        self._subscribe(game.player, 'health_changed', self.on_health_changed)
        self._subscribe(game.inventory, 'inventory_updated', self.on_inventory_updated)
        self._subscribe(game.inventory, 'selected_slot_changed', self.on_selected_slot_changed)
        self._subscribe(game.mob_manager, 'mob_killed', self.on_mob_killed)

        # Инициализация словаря отрисовки предметов
        self._init_item_drawers()

    # ----- Обработчики событий -----
    def on_health_changed(self, event_type, data):
        self.player_hp = data['hp']
        self.player_max_hp = data['max_hp']
        self.hud_dirty = True

    def on_inventory_updated(self, event_type, data):
        self.inventory_slots = self.game.inventory.slots[:]
        self.dragged_slot = self.game.inventory.dragged_slot
        self.inventory_dirty = True
        self.hotbar_dirty = True

    def on_selected_slot_changed(self, event_type, data):
        self.selected_slot = data['slot']
        self.hotbar_dirty = True

    def on_mob_killed(self, event_type, data):
        print(f"[Событие] Моб {data['mob_type']} убит в ({data['x']:.0f}, {data['y']:.0f})")

    # ----- Вспомогательные методы для управления подписками -----
    def _subscribe(self, publisher, event_type, callback):
        """Подписывает и сохраняет для последующей отписки."""
        publisher.subscribe(event_type, callback)
        self._subscriptions.append((publisher, event_type, callback))

    def cleanup(self):
        """Отписывает все обработчики, чтобы объект можно было удалить."""
        for publisher, event_type, callback in self._subscriptions:
            publisher.unsubscribe(event_type, callback)
        self._subscriptions.clear()

    # ----- Инициализация отрисовщиков предметов -----
    def _init_item_drawers(self):
        """Инициализирует словарь функций для отрисовки предметов."""
        self.item_drawers = {}

        # Мечи 
        swords = {
            ITEM_SWORD_WOOD: (141, 110, 99),
            ITEM_SWORD_COPPER: (216, 112, 64),
            ITEM_SWORD_IRON: (220, 235, 245),
            ITEM_SWORD_GOLD: (255, 215, 0),
            ITEM_SWORD_DIAMOND: (0, 229, 255),
            ITEM_SWORD_STONE: (158, 158, 158),
            ITEM_SWORD_SILVER: (180, 200, 210),
            ITEM_SWORD_PLATINUM: (220, 235, 245),
            ITEM_SWORD_MITHRIL: (120, 200, 220),
            ITEM_SWORD_ADAMANTITE: (80, 50, 120),
            ITEM_SWORD_TITAN: (100, 150, 200),
            ITEM_SWORD_COBALT: (50, 150, 200),
            ITEM_SWORD_NETHERITE: (80, 40, 100),
            ITEM_SWORD_CRYSTAL: (200, 240, 255),
        }
        for item_type, color in swords.items():
            self.item_drawers[item_type] = lambda s, it, x, y, sz, c=color: self._draw_sword(s, x, y, sz, c)

        # Кирки
        pickaxes = {
            ITEM_PICKAXE_WOOD: (141, 110, 99),
            ITEM_PICKAXE_COPPER: (216, 112, 64),
            ITEM_PICKAXE_IRON: (220, 235, 245),
            ITEM_PICKAXE_GOLD: (255, 215, 0),
            ITEM_PICKAXE_STONE: (158, 158, 158),
            ITEM_PICKAXE_DIAMOND: (0, 229, 255),
            ITEM_PICKAXE_SILVER: (180, 200, 210),
            ITEM_PICKAXE_PLATINUM: (220, 235, 245),
            ITEM_PICKAXE_MITHRIL: (120, 200, 220),
            ITEM_PICKAXE_ADAMANTITE: (80, 50, 120),
            ITEM_PICKAXE_TITAN: (100, 150, 200),
            ITEM_PICKAXE_COBALT: (50, 150, 200),
            ITEM_PICKAXE_NETHERITE: (80, 40, 100),
            ITEM_PICKAXE_CRYSTAL: (200, 240, 255),
        }
        for item_type, color in pickaxes.items():
            self.item_drawers[item_type] = lambda s, it, x, y, sz, c=color: self._draw_pickaxe(s, x, y, sz, c)
        ...
        for item_type, color in axes.items():
            self.item_drawers[item_type] = lambda s, it, x, y, sz, c=color: self._draw_axe(s, x, y, sz, c)

        # Слитки
        ingots = {
            ITEM_COPPER_INGOT: (216, 112, 64),
            ITEM_IRON_INGOT: (207, 216, 220),
            ITEM_GOLD_INGOT: (255, 215, 0),
            ITEM_SILVER_INGOT: (180, 200, 210),
            ITEM_PLATINUM_INGOT: (220, 235, 245),
            ITEM_MITHRIL_INGOT: (120, 200, 220),
            ITEM_ADAMANTITE_INGOT: (80, 50, 120),
            ITEM_TITAN_INGOT: (100, 150, 200),
            ITEM_COBALT_INGOT: (50, 150, 200),
            ITEM_NETHERITE_INGOT: (80, 40, 100),
            ITEM_CRYSTAL_INGOT: (200, 240, 255),
        }
        for item_type, color in ingots.items():
            self.item_drawers[item_type] = lambda s, it, x, y, sz, c=color: self._draw_ingot(s, x, y, sz, c)

        # Самоцветы
        gems = {
            ITEM_RUBY: (255, 50, 50),
            ITEM_SAPPHIRE: (50, 50, 255),
            ITEM_EMERALD: (50, 255, 50),
            ITEM_AMETHYST: (200, 50, 200),
            ITEM_OPAL: (200, 200, 255),
            ITEM_AMBER: (255, 200, 50),
        }
        for item_type, color in gems.items():
            self.item_drawers[item_type] = lambda s, it, x, y, sz, c=color: self._draw_gem(s, x, y, sz, c)

        # Еда – 
        food_items = [ITEM_APPLE, ITEM_BREAD, ITEM_COOKED_MEAT, ITEM_PIE, ITEM_SOUP,
                      ITEM_MUSHROOM_SOUP, ITEM_COOKED_MUSHROOM, ITEM_PIZZA, ITEM_BURGER,
                      ITEM_SUSHI, ITEM_CAKE, ITEM_SALAD, ITEM_FRIED_POTATOES,
                      ITEM_CARROT_JUICE, ITEM_BANANA, ITEM_ORANGE, ITEM_CARROT,
                      ITEM_POTATO, ITEM_TOMATO, ITEM_CUCUMBER]
        for item_type in food_items:
            self.item_drawers[item_type] = lambda s, it, x, y, sz, itype=item_type: self._draw_food(s, itype, x, y, sz)

        # Оружие (молот, копьё, арбалет)
        self.item_drawers[ITEM_HAMMER] = self._draw_hammer
        self.item_drawers[ITEM_SPEAR] = self._draw_spear
        self.item_drawers[ITEM_CROSSBOW] = self._draw_crossbow
        ...
        for item_type, color in fish_colors.items():
            self.item_drawers[item_type] = lambda s, it, x, y, sz, c=color, itype=item_type: self._draw_fish(s, x, y, sz, c)

        # Музыкальные инструменты – 
        for item_type in [ITEM_GUITAR, ITEM_FLUTE, ITEM_DRUM, ITEM_HARP]:
            self.item_drawers[item_type] = lambda s, it, x, y, sz, itype=item_type: self._draw_music(s, itype, x, y, sz)

        # Фермерские инструменты – 
        for item_type in [ITEM_HOE, ITEM_WATERING_CAN, ITEM_FERTILIZER]:
            self.item_drawers[item_type] = lambda s, it, x, y, sz, itype=item_type: self._draw_farm(s, itype, x, y, sz)

        # Ресурсы –
        resources = [ITEM_GEL, ITEM_COIN, ITEM_COAL, ITEM_DIAMOND, ITEM_TORCH,
                     ITEM_STICK, ITEM_LEATHER, ITEM_STRING, ITEM_FEATHER,
                     ITEM_FLINT, ITEM_BONE, ITEM_BOOK, ITEM_PAPER, ITEM_BOWL,
                     ITEM_MUSHROOM, ITEM_WHEAT, ITEM_LEATHER_STRIP, ITEM_SCALE,
                     ITEM_SHELL, ITEM_FEATHER_BLUE, ITEM_FEATHER_RED,
                     ITEM_FEATHER_GREEN, ITEM_BONE_MEAL, ITEM_WOOL]
        for item_type in resources:
            self.item_drawers[item_type] = lambda s, it, x, y, sz, itype=item_type: self._draw_resource(s, itype, x, y, sz)
        ...
        for item_type, color in spellbooks.items():
            self.item_drawers[item_type] = lambda s, it, x, y, sz, c=color, itype=item_type: self._draw_spellbook(s, x, y, sz, c)

        # Лук, стрелы, удочка, наживка
        self.item_drawers[ITEM_BOW] = self._draw_bow
        self.item_drawers[ITEM_ARROW] = self._draw_arrow
        self.item_drawers[ITEM_FISHING_ROD] = self._draw_fishing_rod
        self.item_drawers[ITEM_BAIT] = self._draw_bait

    # ----- Вспомогательные методы отрисовки предметов (без изменений) -----
    # (все методы _draw_* остаются точно такими же, как были)
    @staticmethod
    def _draw_sword(surface, x, y, size, color):
        cx, cy = x + size // 2, y + size // 2
        pygame.draw.line(surface, color, (x + 8, y + size - 8), (x + size - 5, y + 5), 4)
    ...
    @staticmethod
    def _draw_pickaxe(surface, x, y, size, color):
        pygame.draw.line(surface, (101, 67, 33), (x + 5, y + size - 5), (x + size - 8, y + 8), 3)
        head_pts = [(x + size - 16, y + 4), (x + size - 4, y + 4), (x + size - 4, y + 16)]
        pygame.draw.lines(surface, color, False, head_pts, 4)
        pygame.draw.circle(surface, color, (x + size - 4, y + 4), 3)

    @staticmethod
    def _draw_axe(surface, x, y, size, color):
        pygame.draw.line(surface, (101, 67, 33), (x + 5, y + size - 5), (x + size - 8, y + 8), 3)
        pygame.draw.polygon(surface, color, [(x + size - 14, y + 4), (x + size - 4, y + 10), (x + size - 4, y + 20)])

    @staticmethod
    def _draw_ingot(surface, x, y, size, color):
        pygame.draw.polygon(surface, color, [(x + 6, y + 20), (x + 10, y + 10), (x + 22, y + 10), (x + 26, y + 20)])

    @staticmethod
    def _draw_gem(surface, x, y, size, color):
        cx, cy = x + size // 2, y + size // 2
        pygame.draw.circle(surface, color, (cx, cy), 8)
        pygame.draw.circle(surface, (255, 255, 255), (cx - 2, cy - 2), 3)

    @staticmethod
    def _draw_food(surface, item_type, x, y, size):
        cx, cy = x + size // 2, y + size // 2
    ...
    @staticmethod
    def _draw_hammer(surface, x, y, size):
    ...
    @staticmethod
    def _draw_spear(surface, x, y, size):
    ...
    @staticmethod
    def _draw_crossbow(surface, x, y, size):
    ...
    @staticmethod
    def _draw_fish(surface, x, y, size, color):
        pygame.draw.ellipse(surface, color, (x + 6, y + 10, size - 12, size - 16))
        pygame.draw.polygon(surface, (200, 200, 200), [(x + size - 6, y + 14), (x + size - 2, y + 10), (x + size - 2, y + 18)])

    @staticmethod
    def _draw_music(surface, item_type, x, y, size):
    ...
    @staticmethod
    def _draw_farm(surface, item_type, x, y, size):
    ...
    @staticmethod
    def _draw_resource(surface, item_type, x, y, size):
        cx, cy = x + size // 2, y + size // 2
        if item_type == ITEM_GEL:
            ...
            colors = {ITEM_FEATHER_BLUE: (50, 50, 255), ITEM_FEATHER_RED: (255, 50, 50),
                      ITEM_FEATHER_GREEN: (50, 255, 50)}
    ...
    @staticmethod
    def _draw_spellbook(surface, x, y, size, color):
        pygame.draw.rect(surface, color, (x + 6, y + 6, size - 12, size - 12), 0)
        pygame.draw.rect(surface, (200, 200, 200), (x + 8, y + 8, size - 16, size - 16), 1)

    @staticmethod
    def _draw_bow(surface, x, y, size):
    ...
    @staticmethod
    def _draw_arrow(surface, x, y, size):
        pygame.draw.line(surface, (121, 85, 72), (x + 4, y + size // 2), (x + size - 4, y + size // 2), 2)
        pygame.draw.polygon(surface, (200, 200, 200),
                            [(x + size - 4, y + size // 2), (x + size - 10, y + size // 2 - 4),
                             (x + size - 10, y + size // 2 + 4)])

    @staticmethod
    def _draw_fishing_rod(surface, x, y, size):
    ...
    @staticmethod
    def _draw_bait(surface, x, y, size):
        cx, cy = x + size // 2, y + size // 2
        pygame.draw.circle(surface, (150, 100, 50), (cx, cy), 5)

    # ----- Основной метод отрисовки иконки -----
    def draw_item_icon(self, surface, item_type, x, y, size=32):
        if item_type in BLOCK_COLORS:
            if item_type == BLOCK_TALL_GRASS:
                ...
                return
            pygame.draw.rect(surface, BLOCK_COLORS.get(item_type, (85, 85, 85)),
                             (x + 2, y + 2, size - 4, size - 4))
            pygame.draw.rect(surface, (20, 20, 20), (x + 2, y + 2, size - 4, size - 4), 1)
            return

        drawer = self.item_drawers.get(item_type)
        if drawer:
            drawer(surface, item_type, x, y, size)
        else:
            cx, cy = x + size // 2, y + size // 2
    ...
    def draw_player(self, px, py):
        ...
        face_dir = 1 if player.facing_right else -1
        leg_step = math.sin(player.anim_frame) * 5 if player.is_grounded and player.vx != 0 else 0
        ...
        hand_y = py - 2
        swing_progress = player.swing_anim / 0.15 if player.swing_anim > 0 else 0
        swing_angle = (1 - swing_progress) * 80 if player.swing_anim > 0 else 0
        ...
        if curr_item != BLOCK_AIR:
            ...
            self.draw_item_icon(item_surf, curr_item, 0, 0, size=32)
            if not player.facing_right:
                item_surf = pygame.transform.flip(item_surf, True, False)
            rotated_item = pygame.transform.rotate(item_surf, -swing_angle if player.facing_right else swing_angle)
    ...
    # ----- Хотбар (использует локальные копии) -----
    def draw_hotbar(self):
        ...
        for i in range(10):
            ...
            pygame.draw.rect(self.screen, (58, 80, 107), (x, y, 44, 44),
                             2 if i != self.selected_slot else 3)
            if item['type'] != BLOCK_AIR:
                self.draw_item_icon(self.screen, item['type'], x + 6, y + 6, size=32)
                if item['count'] > 1:
                    cnt = self.font_small.render(str(item['count']), True, (255, 255, 255))
                    self.screen.blit(cnt, (x + 22, y + 25))

    # ----- Инвентарь (использует локальные копии) -----
        ...
        for row in range(3):
            for col in range(10):
                ...
                if item['type'] != BLOCK_AIR and not is_selected:
                    self.draw_item_icon(self.screen, item['type'], x + 5, y + 5, size=32)
                    if item['count'] > 1:
                        cnt = self.font_small.render(str(item['count']), True, (255, 255, 255))
    ...
    def render(self):
        ...
        self.draw_entities()
        self.draw_hud()
        self.game.weather.draw_weather(self.screen)
        ...
        if self.dragged_slot is not None:
            item = self.inventory_slots[self.dragged_slot]
            if item['type'] != BLOCK_AIR:
                self.draw_item_icon(self.screen, item['type'],
                                    self.game.mouse_x - 16, self.game.mouse_y - 16, size=32)

    # ----- Подзадачи render (фон, мир, сущности, HUD, меню) -----
    def draw_background(self):
        ...
        self.game.weather.draw_clouds(self.screen)
    ...
    def draw_entities(self):
        ...
        for item in self.game.mob_manager.dropped_items:
            ...
            self.draw_item_icon(self.screen, item.item_type, int(sx - 12), int(sy - 12), size=24)
        ...
        for mob in self.game.mob_manager.mobs:
            ...
            if hp_pct > 0:
                pygame.draw.rect(self.screen, (118, 255, 3),
                                 (sx - bar_w // 2, sy - mob.h // 2 - 10, bar_w * hp_pct, 4))
    ...
    def draw_hud(self):
        ...
        hp_text = self.font_small.render(f"HP: {self.player_hp} / {self.player_max_hp}", True, (255, 255, 255))
