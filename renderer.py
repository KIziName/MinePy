...
class GameRenderer:
    def __init__(self, game):
        ...
        # Словарь для быстрой отрисовки предметов
    ...
    def _init_item_drawers(self):
        ...
        for item_type in food_items:
            self.item_drawers[item_type] = lambda s, it, x, y, sz: self._draw_food(s, it, x, y, sz)
        ...
        for item_type, color in fish_colors.items():
            self.item_drawers[item_type] = lambda s, it, x, y, sz, c=color: self._draw_fish(s, x, y, sz, c)

        # Музыкальные инструменты
        for item_type in [ITEM_GUITAR, ITEM_FLUTE, ITEM_DRUM, ITEM_HARP]:
            self.item_drawers[item_type] = lambda s, it, x, y, sz: self._draw_music(s, it, x, y, sz)

        # Фермерские инструменты
        for item_type in [ITEM_HOE, ITEM_WATERING_CAN, ITEM_FERTILIZER]:
            self.item_drawers[item_type] = lambda s, it, x, y, sz: self._draw_farm(s, it, x, y, sz)
        ...
        for item_type in resources:
            self.item_drawers[item_type] = lambda s, it, x, y, sz: self._draw_resource(s, it, x, y, sz)
        ...
        for item_type, color in spellbooks.items():
            self.item_drawers[item_type] = lambda s, it, x, y, sz, c=color: self._draw_spellbook(s, x, y, sz, c)

        self.item_drawers[ITEM_SCROLL_TELEPORT] = self._draw_scroll

        # Остальное (лук, стрелы, удочка, наживка)
    ...
    @staticmethod
    def _draw_scroll(surface, x, y, size):
    ...
    def draw_item_icon(self, surface, item_type, x, y, size=32):
        """Отрисовывает предмет по его типу."""
    ...
    def draw_hotbar(self):
        ...
        for i in range(10):
            ...
            pygame.draw.rect(self.screen, (58, 80, 107), (x, y, 44, 44),
                             2 if i != self.game.inventory.selected_slot else 3)
    ...
    def render(self):
        ...
        self.draw_entities()
        self.draw_hud()
        self.game.weather.draw_weather(self.screen)  # погода поверх всего
