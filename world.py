...
class GameWorld:
            ...
            try:
                with self.lock:
                    if chunk_x in self.chunk_data:
                        self.task_queue.task_done()
                        continue
                data = self._generate_chunk_data(chunk_x)   # np.ndarray
                with self.lock:
    ...
    def _generate_chunk_data(self, chunk_x):
        # Создаём пустой чанк (воздух) - форма (H, W)
        chunk = np.full((WORLD_HEIGHT, CHUNK_WIDTH), BLOCK_AIR, dtype=np.int16)

        # 1. Базовый ландшафт (трава, земля, камень)
        for local_x in range(CHUNK_WIDTH):
            ...
            # Земля (слой под травой)
            start_soil = max(0, ground_h - DIRT_LAYER_THICKNESS)
            chunk[start_soil:ground_h, local_x] = BLOCK_DIRT

            # Всё, что глубже земли, пока камень (потом заменим рудами)
            chunk[:start_soil, local_x] = BLOCK_STONE

        # 2. Генерация руд (векторизованно)
    ...
    def load_data(self, chunks_data):
        ...
        with self.lock:
            ...
            self.land_height_cache.clear()
            self.requested.clear()
            # chunks_data приходит из JSON как dict: str(chunk_x) -> list[list[int]]
            # Преобразуем в np.array
            self.chunk_data = {
                int(k): np.array(v, dtype=np.int16) for k, v in chunks_data.items()
            }
