...
class GameWorld:
    ...
    def _worker_loop(self):
        while self.running:
            ...
            # Проверяем, не сигнал ли это остановки
            if chunk_x is None:
                self.task_queue.task_done()
                break

            # Проверяем, не появился ли чанк уже (синхронно или в другом потоке)
            with self.lock:
    ...
    def ensure_chunk(self, chunk_x):
        ...
        with self.lock:
            if self.running and chunk_x not in self.chunk_data:
                self.chunk_data[chunk_x] = data
                self.dirty_chunks.add(chunk_x)
                self.requested.discard(chunk_x)  # убираем из запрошенных, если была
