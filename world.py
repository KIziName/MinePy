...
class GameWorld:
    ...
    def _generate_chunk_data(self, chunk_x):
        # ----- Используем глобальное зерно, если оно задано -----
        if WORLD_SEED is not None:
            # Комбинируем глобальное зерно с номером чанка для уникальности
            seed = WORLD_SEED + chunk_x * 1234567 + 9876543
        else:
            seed = chunk_x * 1234567 + 9876543
        ...
        rand = rng_np.random((WORLD_HEIGHT, CHUNK_WIDTH))
        ...
        for local_x in range(CHUNK_WIDTH):
            ...
            if rng.random() < DECORATION_CHANCE:
                if rng.random() < GRASS_CHANCE:
                    chunk[ground_h, local_x] = BLOCK_TALL_GRASS
                else:
                    chunk[ground_h, local_x] = rng.choice(FLOWER_TYPES)

            if rng.random() < TREE_CHANCE and ground_h > 8:
                tree_h = rng.randint(TREE_MIN_HEIGHT, TREE_MAX_HEIGHT)
    ...
    def request_chunk(self, chunk_x):
        with self.lock:
            ...
            if WORLD_MAX_X is not None and chunk_x * CHUNK_WIDTH > WORLD_MAX_X:
                return
    ...
    def ensure_chunk(self, chunk_x):
        """Синхронная загрузка чанка (используется только для set_block)"""
        with self.lock:
            # Проверка на максимальную координату
            if WORLD_MAX_X is not None and chunk_x * CHUNK_WIDTH > WORLD_MAX_X:
                return
    ...
    def get_block(self, global_gx, gy):
        """Асинхронный доступ – не ждёт генерации, возвращает BLOCK_AIR если чанка нет"""
        ...
        with self.lock:
            ...
            if data is None:
                if chunk_x not in self.requested:
                    self.requested.add(chunk_x)
                    self.task_queue.put(chunk_x)
                return BLOCK_AIR
    ...
    def set_block(self, global_gx, gy, block_type):
        """Синхронная установка блока – гарантирует наличие чанка"""
