    ...
    def draw_weather(self, screen):
        if self.weather == WEATHER_RAIN and self.rain_particles:
            ...
            for x, y, speed in self.rain_particles:
                pygame.draw.line(surf, (180, 200, 255, alpha), (x, y), (x - 2, y + 10), 1)
            screen.blit(surf, (0, 0))
        elif self.weather == WEATHER_SNOW and self.snow_particles:
            surf = pygame.Surface((self.screen_width, self.screen_height), pygame.SRCALPHA)
            alpha = int(255 * self.weather_alpha)
            for x, y, size, speed in self.snow_particles:
                pygame.draw.circle(surf, (255, 255, 255, alpha), (int(x), int(y)), int(size))
            screen.blit(surf, (0, 0))
...
class Game:
    def __init__(self):
        ...
        self.inventory = GameInventory()                     # <- выделенный класс
        self.mob_manager = GameMobManager(self.world)       # <- выделенный класс
        self.weather = WeatherManager(self.screen_width, self.screen_height)  # <- новый класс
        ...
        # теперь звёзды и фоны оставляем здесь
        self._init_stars()
        self.bg_surfaces = {}
    ...
    # ---------- методы инициализации (без изменений, кроме удаления облаков/частиц) ----------
    ...
    def update_physics(self, dt):
        ...
        # теперь обновляем погоду через менеджер
