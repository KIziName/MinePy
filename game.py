...
class GamePlayer:
    def __init__(self, world):
        ...
        self.swing_anim = 0.0
        self.damage_bonus = 0   # только для колец/украшений
    ...
    def update(self, keys, dt):
        self.vx = 0
        speed = PLAYER_SPEED   # бонус скорости удалён
        ...
        if (keys.get(pygame.K_w) or keys.get(pygame.K_SPACE) or keys.get(pygame.K_UP)) and self.is_grounded:
            self.vy = JUMP_FORCE   # бонус прыжка удалён
            self.is_grounded = False
...
class Game:
    ...
    def handle_game_click(self, pos, button=1):
        ...
        if button == 1:
            # Левая кнопка – только атака (удар по мобу и разрушение блока)
                ...
                if abs(mob.x - wx) < 30 and abs(mob.y - wy) < 30:
                    ...
                    if mob.hp <= 0:
                        ...
                        if isinstance(mob, Slime):
                            ...
                            drop_type, count = ITEM_GEL, 1  # вместо ITEM_LENS
            ...
            # Правая кнопка – использование предметов и размещение блоков
            slot = self.inventory.get_selected_item()
