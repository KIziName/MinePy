...
class Game:
    ...
    def reset_game_data(self):
        """Полная очистка игрового состояния с пересозданием всех объектов.
           Старые объекты будут удалены сборщиком мусора, т.к. подписки используют слабые ссылки.
        """
        self.inventory_open = False
        self.pause_menu_open = False
        self.day_time = 3000
        ...
        # Останавливаем поток генерации старого мира
        self.world.stop()

        # Создаём новый мир, менеджер мобов, инвентарь, игрока
        self.world = GameWorld()
        self.mob_manager = GameMobManager(self.world)
        self.inventory = GameInventory()
        self.player = GamePlayer(self.world)
        ...
        # Пересоздаём погоду, звёзды, фон
        self.weather = WeatherManager(self.screen_width, self.screen_height)
        self._init_stars()
        self._build_background_surfaces()

        # Пересоздаём рендерер – старый будет удалён сборщиком
        self.renderer = GameRenderer(self)
